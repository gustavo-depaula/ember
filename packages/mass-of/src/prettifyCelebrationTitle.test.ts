...
describe('prettifyCelebrationTitle', () => {
  ...
  it('strips "<Ordinal> semana " prefix on solemnities falling on weekdays', () => {
    expect(
      prettifyCelebrationTitle({ 'pt-BR': 'Sexta semana ASCENÇÃO DO SENHOR' }, '')['pt-BR'],
    ).toBe('Ascenção do Senhor')
  })
  ...
  it('disambiguates the four Christmas Masses', () => {
    const t = { 'pt-BR': 'NATAL DO SENHOR', en: 'The Nativity of the Lord' }
    expect(prettifyCelebrationTitle(t, 'tempore.christmas.nativity-vigil')['pt-BR']).toBe(
      'Natal do Senhor — Missa da Vigília',
    )
    expect(prettifyCelebrationTitle(t, 'tempore.christmas.nativity-night')['pt-BR']).toBe(
      'Natal do Senhor — Missa da Noite',
    )
    expect(prettifyCelebrationTitle(t, 'tempore.christmas.nativity-dawn')['pt-BR']).toBe(
      'Natal do Senhor — Missa da Aurora',
    )
    expect(prettifyCelebrationTitle(t, 'tempore.christmas.nativity-day')['pt-BR']).toBe(
      'Natal do Senhor — Missa do Dia',
    )
    expect(prettifyCelebrationTitle(t, 'tempore.christmas.nativity-vigil')['en-US']).toBe(
      'The Nativity of the Lord — Mass at the Vigil',
    )
  })
  ...
  it('preserves Latin titles untouched', () => {
    expect(prettifyCelebrationTitle({ la: 'IN NATIVITATE DOMINI' }, '').la).toBe(
