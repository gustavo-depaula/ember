...
- **`pt-BR/hymns-c05.md` H1 didn't match its own TOC entry.** Every chapter's H1 in this book is the vernacular incipit — the parenthetical half of its `book.json` `toc` title — in all three languages. c05's pt-BR H1 was the *practice's* title (`# Adoro-Te Devotamente`, from `content/practices/adoro-te-devote/manifest.json`) rather than the incipit, while `book.json` gave "Adoro Te Devote (Eu Vos Adoro com Devoção)". Changed the H1 to `# Eu Vos Adoro com Devoção`, which is also verbatim how the chapter's own first line opens. Fixed in the chapter rather than in `book.json` because the TOC entry already agreed with the body text and with the other five chapters — the H1 was the lone outlier. The build's H1-drift check never caught this: it only warns when two multi-word titles share *no* words, and "Adoro-Te Devotamente" shares "adoro"/"te" with the TOC title.
- **`en-US/hymns-c04.md`: "annuled" → "annulled."** A dropped letter, inconsistent with the correctly doubled participle in the very next line of the same couplet ("shadows are in truths fulfilled").
- **Corrupt Latin in the Sacris Solemniis closing stanza.** `la/hymns-c02.md` read `sicut nos visitas, sicut te colimus` — an indicative ("as you visit us") where the stanza requires the imperative, and which also destroys the `sic … sicut` correlative. The received text is `sic nos tu visita, sicut te colimus` ("so do thou visit us, as we worship thee"), which is what `content/practices/sacris-solemniis` and `content/practices/panis-angelicus` both carry. Decisively, *this book's own two translations render an imperative* — pt-BR "**visitai-nos** assim como Vos veneramos" and en-US "**o visit us** right soon, as we our homage pay" — neither of which can come from `visitas`. Corrected to `sic nos tu visita`.
- **`la/hymns-c04.md`: `coheredes` → `cohaeredes`.** Orthographic normalization rather than a grammatical correction: the file preserves the `ae` diphthong everywhere else (`praesignatur`, `sacrae`, `mensae`, `inaequali`, `Paschae`), and `content/practices/lauda-sion` and `content/practices/little-office-most-holy-sacrament-of-the-eucharist` both spell it `cohaeredes`. Flagging the reasoning here in case the intent was to preserve a medieval `e`-for-`ae` spelling — but nothing else in the file does that.
- **`en-US/hymns-c05.md`: "call you Lord and God as he" → "call thee Lord and God as he."** Hopkins's translation is uniformly archaic in its second person — eleven instances of *thee* / *thou* / *thy* against this single `you`, in the middle of a line whose own next clause reads "as he". Hopkins's published text reads *thee*; the scrape had modernized exactly one pronoun.
- **`la/hymns-c04.md`: `coena` → `cena`.** Another orthographic outlier: the same file spells it `cenae` twelve lines earlier ("in sacrae mensa cenae"), `la/hymns-c01.md` has "In supremae nocte cenae", and `content/practices/lauda-sion` uses `cena`/`cenae`. Both spellings are attested Latin, but nothing else in this book uses the `oe` form.
- **Not changed: `tu nos ibi commensales` in `la/hymns-c04.md`.** The Roman Missal reads `tuos ibi commensales`, but this file carries its own inline apparatus note recording exactly that ("BNF 1143 reads *tuos* instead of *tu nos*"), so `tu nos` is this recension's deliberate editorial reading, not a scrape corruption. Left alone — the same way this recension prints only `Amen.` where the Missal closes `Amen. Alleluia.`
- **Corrupt Latin in the Pange Lingua doxology.** `la/hymns-c01.md` closed with `procedentis ab utroque` — a genitive where the stanza requires a dative. The doxology addresses three recipients of praise in the dative (`Genitori, Genitoque … procedenti ab utroque`: "to the Father, and to the Son, … to the One proceeding from both, let equal praise be"). Every other copy of this text in the corpus — `content/practices/pange-lingua`, `content/practices/tantum-ergo`, and `content/practices/little-office-blessed-sacrament` — already reads `procedenti`, and both translations here render it as a dative (`ao que d'Ambos procede`; "with the Holy Ghost proceeding forth"). The scraped book file was the sole outlier. Corrected to `procedenti`.

Also inherited, so deliberately **not** changed: `pt-BR/hymns-c05.md`'s last line reads `na visão da vossa glória` in lowercase, where the rest of the chapter capitalizes reverential second-person forms (`Vós`, `Vos`, `Vosso Sangue`). It is verbatim from `content/practices/adoro-te-devote/manifest.json`, so it falls under the same rule as the c01–c04 inherited wordings above: if it should change, the practice manifest changes first, so book and practice move together.
