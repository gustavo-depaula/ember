...
A fourth review found one defect. Like the third round's, it is invisible to a fidelity reading — but where that one was punctuation inherited from the source's typesetting, this one is Portuguese grammar applied to a word the source left in Latin:

| Was | Now | Why |
|-----|-----|-----|
| **a clara** *rationale* do fato | **o claro** *rationale* do fato | *Rationale* is the substantivized neuter of *rationalis, -e*, and Portuguese assigns masculine gender to unassimilated Latin neuters and indeclinables. The corpus agrees wherever it has an opinion: `o seu *fiat*`, `o *Magnificat*`, `o *Credo*`, `um bom *peccavi*`, `o *Enchirídion*`, `o *Te Deum*` — all masculine; Latin feminines keep feminine (`uma *societas*`); work titles take the gender of the implied genre noun (`a *Mediator Dei*` = a encíclica, `uma *summa theologica*` = a suma). No route gives feminine: even read as the English borrowing "rationale", an English noun entering Portuguese defaults to masculine. The feminine is leakage from *razão*, the Portuguese word the translator was thinking through — agreement with the concept instead of with the word on the page. |

**This is the only Portuguese article + italicized Latin noun of its shape in the corpus**, so there was no sibling to copy and no sibling to contradict — the convention had to be derived from the neuter-to-masculine pattern above rather than looked up. That is the general case for a *hapax* construction, and it is why the measurement went wide (every `article + *italic*` pair in every pt-BR file under `content/books/`) instead of deep.

Note that the sentence's other four Latin terms are all parenthetical glosses (`curador (*curatorem*)`, `unidade (*unita est*)`, `formada no ar (*aere efformatus*)`, `iguala (*aequiparat*)`), where the Portuguese head noun carries the gender and the Latin never touches the grammar. *Rationale* is the one term the source uses bare, inside its own sentence — so it is the one place where keeping the source's Latin forces a Portuguese agreement decision. **A bare inline Latin noun inherits the target language's agreement rules; a glossed one does not.**

Three candidates raised in this round were examined and **ruled out** as non-defects:

- `ele se iguala (*aequiparat*)` for "He supplies in the same equality" — re-raised by a fresh cold bilingual reader, and ruled OUT for the same reason as in round three. The reader reached it independently and conceded in the same breath that the Portuguese is "closer to what the Latin actually says and theologically safer", which is the round-three argument restated. **A candidate that a cold reader keeps re-finding is not thereby a defect; it is a place where the source's own wording is odd**, and the journal entry is what stops each round from re-deciding it.
- `a quem podemos considerá-lo semelhante` as a *pronome lembrete* — also re-raised, this time by the monolingual pass, and also ruled OUT on round three's grounds: `-lo` is the direct object of *considerar*, `a quem` the `a`-marked complement of the predicative *semelhante*, and *considerar* does not take an `a`-marked direct object, so the two cannot be filling one slot.
- Colon-count asymmetry (12 pt-BR against 10 en-US), where Portuguese writes `dizendo:` and `com certeza:` for English commas. Introducing reported discourse with a colon is standard Portuguese and already attested elsewhere in this same file. Colon parity was never an invariant here; as recorded in round three, structural parity is a check for dropped content, not a rule that punctuation must be transplanted.

Verification after the fix: mechanical re-check (43/43 sentence alignment, 3/3 paragraphs, 14/14 italic spans, 5/5 parentheses, 15/15 semicolons, 3/3 quotation pairs, 9/10 em-dashes as documented above, all five Latin terms byte-identical, Scripture chapter:verse identical, H1s matching `book.json`, no encoding artifacts), a fresh cold bilingual read, and a monolingual pt-BR proofread — all three clean. `pnpm build:corpus` clean, no H1/TOC drift warning for this book.
