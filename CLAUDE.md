...
- **ContentSource:** data fetcher that returns one or more primitives. Lives in `apps/app/src/producers/` + `packages/producers/`. Has `id` / `version` / `prefsDeps` (declarative cache deps) / `fetch(ctx)`. No `kind` discriminator. Composable via the injected `sources` accessor.
