 ...
 *
 * Outputs:
 *   - content/practices/saint-of-the-day/data/saint-of-day-index.json — bilingual
 *     per-day lookup (name + primary chapter + first chapter's reflection),
 *     served by Hearth and fetched on demand by the home/Explore cards.
...
const dataOutPath = resolve(
  root,
  'content/practices/saint-of-the-day/data/saint-of-day-index.json',
)
...
for (const option of dateSelect.options) {
  ...
  const entry = { name, chapter: chapterWithImage }
  if (reflection) {
    entry.reflection = reflection
    withReflection++
  }
  indexEntries[dateKey] = entry
...
mkdirSync(dirname(dataOutPath), { recursive: true })
writeFileSync(dataOutPath, `${JSON.stringify(indexEntries, null, 2)}\n`, 'utf8')

const dayCount = Object.keys(indexEntries).length
console.log(
  `Wrote ${dayCount} days to ${dataOutPath} and rewrote ${flowPath} ` +
    `(${withReflection} days with reflection).`,
)
