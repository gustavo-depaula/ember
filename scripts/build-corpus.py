...
def build_do(b: Builder) -> None:
    ...
    content/do is the upstream repo as a pinned git submodule; the files ship
    verbatim from its web/www. The import scope here mirrors
    scripts/validate-do.ts — change both together.
    ...
    repo = CONTENT / "do"
    www = repo / "web" / "www"
    if not (www / "Tabulae").is_dir():
        # Shipping a corpus without DO would silently break the EF Mass and
        # the Office, so an uninitialized submodule is fatal, not skipped.
        raise SystemExit("content/do is empty: run `git submodule update --init content/do`")

    def git_head(fmt: str) -> str:
        return subprocess.run(
            ["git", "log", "-1", f"--format={fmt}"], cwd=repo, check=True, capture_output=True, text=True
        ).stdout.strip()

    meta = {"repo": "DivinumOfficium/divinum-officium", "commit": git_head("%H"), "commitDate": git_head("%cI")}

    # file id -> upstream path. The id is the path without `.txt`; the four
    # extensionless dialog/setup files keep their bare name.
    sources: dict[str, Path] = {}

    def add_tree(d: Path) -> None:
        for f in d.rglob("*.txt"):
            sources[f.relative_to(www).as_posix()[: -len(".txt")]] = f

    for lang in DO_LANG_DIRS:
        for d in DO_HORAS_DATASETS:
            add_tree(www / "horas" / lang / d)
        for d in DO_MISSA_DATASETS:
            add_tree(www / "missa" / lang / d)
    add_tree(www / "horas" / "Ordinarium")
    for name in ("horas/horas.dialog", "horas/horas.setup", "missa/missa.dialog", "missa/missa.setup"):
        sources[name] = www / name
    sources["Tabulae/data"] = www / "Tabulae" / "data.txt"
    for d in ("Kalendaria", "Transfer", "Stransfer", "Tempora"):
        add_tree(www / "Tabulae" / d)

    localized: dict[str, dict[str, dict[str, dict]]] = {}  # dataset -> file id -> lang -> ref
    plain: dict[str, dict[str, dict]] = {}  # dataset -> file id -> ref

    for id_path, f in sorted(sources.items()):
        data = f.read_bytes()
        try:
            data.decode("utf-8")
        except UnicodeDecodeError as e:
            raise ValueError(f"content/do: {id_path} is not valid UTF-8") from e
        parts = id_path.split("/")
        # The blob is the raw upstream text; the engine parses it on read
        # (parseDoFile).
        ref = dict(zip(("hash", "size"), b.write_blob(data)))
        if parts[0] == "Tabulae":
            ...
            if lang is None or dataset is None:
                raise ValueError(f"unexpected content/do path: {id_path}")
