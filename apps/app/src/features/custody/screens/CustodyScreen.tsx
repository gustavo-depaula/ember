...
import { commitmentHref, newCommitmentHref } from '../routes'
import { COMMITMENT_TEMPLATES, type CommitmentTemplate } from '../templates'
          ...
          <YStack gap="$sm">
            {commitments?.map((c) => (
              <CommitmentRow
                key={c.id}
                commitment={c}
                onPress={() => router.push(commitmentHref(c.id))}
              />
            ))}
          </YStack>
        )}
...
function TemplateLink({ template }: { template: CommitmentTemplate }) {
  return (
    <Link href={newCommitmentHref(template.id)} push asChild onPress={() => lightTap()}>
      <Link.AppleZoom>
        <AnimatedPressable
...
function CustomLink({ label }: { label: string }) {
  return (
    <Link href={newCommitmentHref()} push asChild onPress={() => lightTap()}>
      <Link.AppleZoom>
        <AnimatedPressable
          accessibilityRole="link"
