...
type Props = {
  html: string
  onMessage?: (msg: EpubMessage) => void
}
...
const NativeWebView = forwardRef<EpubWebViewHandle, Props>(function NativeWebView(
  { html, onMessage },
  ref,
) {
  ...
  const handleMessage = useCallback(
    (event: { nativeEvent: { data: string } }) => {
      try {
        onMessage?.(JSON.parse(event.nativeEvent.data) as EpubMessage)
      } catch {}
    },
    [onMessage],
  )
...
const WebIframe = forwardRef<EpubWebViewHandle, Props>(function WebIframe(
  { html, onMessage },
  ref,
) {
...
export const EpubWebView = forwardRef<EpubWebViewHandle, Props>(function EpubWebView(props, ref) {
