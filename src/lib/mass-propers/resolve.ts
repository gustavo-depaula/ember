...
import type { DayCalendar, LiturgicalCategory } from '@/lib/liturgical'
import { getDoSanctiId, getDoTemporaId } from './do-file-id'
import { hasSancti, hasTempora, loadSancti, loadTempora } from './propers-data'
 ...
 */
export function getProperDay(date: Date, dayCalendar?: DayCalendar): ProperDay | undefined {
  const raw = loadRawProperDay(date, dayCalendar)
  if (!raw) return undefined

  const result: ProperDay = {}
  for (const [sectionId, section] of Object.entries(raw)) {
    const text = localizeContent({ en: section.en ?? '', 'pt-BR': section['pt-BR'] })
    if (!text) continue
    result[sectionId] = {
      text,
      latin: section.latin,
...
type RawSection = { en?: string; latin?: string; 'pt-BR'?: string; citation?: string }
type RawProperFile = Record<string, RawSection>

function loadRawProperDay(date: Date, dayCalendar?: DayCalendar): RawProperFile | undefined {
  const source = chooseProperSource(date, dayCalendar)
  const temporaId = getDoTemporaId(date)
  const sanctiId = getDoSanctiId(date)

  const tempora = temporaId && hasTempora(temporaId) ? loadTempora(temporaId) : undefined
  const sancti = hasSancti(sanctiId) ? loadSancti(sanctiId) : undefined
