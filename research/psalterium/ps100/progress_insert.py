"""Insert ps100's PROGRESS row in psalm order (after the last row numbered below 100). Re-reads and writes in one go."""
import json
import re
...
if any(line.startswith('| 100 |') for line in lines):
    print('already there')
    raise SystemExit
at = None
