"""Insert Ps 73's row into PROGRESS.md after the row of the highest psalm below 73 (reads and writes in one go)."""
import re
...
if any(line.startswith('| 73 |') for line in lines):
    print('already there')
    raise SystemExit
best, at = -1, None
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and best < int(m.group(1)) < 73:
