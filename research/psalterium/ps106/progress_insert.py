"""Insert ps106's PROGRESS row in psalm order (after the last row numbered below 106, past the header). Re-reads and writes in one go."""
import json
import re
...
if any(line.startswith('| 106 |') for line in lines):
    print('already there')
    raise SystemExit
at = None
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and int(m.group(1)) < 106 and i > 10:
        at = i
