"""Latin regex (accent-folded) -> each finished psalm's verse with its prayed Portuguese (canticle 211 lookups).
Copied from ps211/concord.py.
python3.13 research/psalterium/ps211/concord.py '<regex>' [...]"""
import json
import re
import sys
import unicodedata
...
for folder in sorted(root.glob('ps[0-9][0-9][0-9]')):
    ...
    if not lat.exists() or not vos.exists() or folder.name == 'ps211':
        continue
