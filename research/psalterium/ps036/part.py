"""Critic inputs for a verse range of Ps 36, so that the blind readers read only the new portion.
Copied from ps036/part.py (left untouched) and adapted: the range is given as DO verse numbers.

Run from the repo root, after render.py and partial.py:
  python3.13 research/psalterium/ps036/part.py 21 40 part2      (36:21–36:40 → ps036/part2/)
...
(the Portuguese alone, for the ambiguity reader). Then, one at a time (k = the version the critics read):
  python3.13 research/psalterium/codex.py research/psalterium/ps036/part2 ../../prompts/latinist.md prayed.vos.json ../critic/v<k>.latinist.part2.json
  python3.13 research/psalterium/codex.py research/psalterium/ps036/part2 ../../prompts/stylist.md prayed.vos.json ../critic/v<k>.stylist.part2.json
  python3.13 research/psalterium/codex.py research/psalterium/ps036/part2/blind ../../../prompts/ambiguity.md prayed.vos.json ../../critic/v<k>.ambiguity.part2.json
...
# the full Latin, not ps036/latin.json (partial.py trims that one to what is translated)
fullLatin = {v['id']: v['text'] for v in latinModule.readVerses(latinModule.doLatin / 'Psalmorum/Psalm36.txt')}
