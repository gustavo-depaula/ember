"""Insert ps102's PROGRESS row in psalm order (after the last row numbered below 102, among rows 1–150). Re-reads and writes in one go."""
import json
import re
...
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and int(m.group(1)) < 102 and i > 10:
        at = i
