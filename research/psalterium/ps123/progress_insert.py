"""Insert ps123's PROGRESS row in psalm order (after the last row numbered below 123, past the header). Re-reads and writes in one go."""
import json
import re
...
if any(line.startswith('| 123 |') for line in lines):
    print('already there')
    raise SystemExit
at = None
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and int(m.group(1)) < 123 and i > 10:
        at = i
assert at is not None
lines.insert(at + 1, row)
progress.write_text('\n'.join(lines), encoding='utf-8')
print('inserted after line', at + 1, ':', lines[at][:12])
