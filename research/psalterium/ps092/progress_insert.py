"""Insert ps092's PROGRESS row in psalm order (after the last row numbered below 92, among rows 1–150). Re-reads and writes in one go."""
import json
import re
...
if any(line.startswith('| 92 |') for line in lines):
    print('already there')
    raise SystemExit
at = None
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and int(m.group(1)) < 92 and i > 10:
        at = i
