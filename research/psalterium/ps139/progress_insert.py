"""Insert Ps 139's row into PROGRESS.md after the row of the highest psalm below 139 in the 1–150 run
...
if any(line.startswith('| 139 |') for line in lines):
    print('already there')
    raise SystemExit
...
for i, line in enumerate(lines[start:], start):
    m = re.match(r'\| (\d+) \|', line)
    if m and best < int(m.group(1)) < 139:
