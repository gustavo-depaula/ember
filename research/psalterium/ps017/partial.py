"""Checks and critic inputs for Ps 17 while it is PARTIAL — copied from ps009/partial.py (itself from ps118/partial.py; both left untouched) and adapted.

Run from the repo root, after render.py:  python3.13 research/psalterium/ps017/partial.py

Why: checks.py compares prayed.json against all 54 Latin verses and crashes on the first untranslated one;
render.py writes all 54 Latin verses into latin.json, which the blind Latinist would read as missing translations.
This script
  1. trims ps017/latin.json to the verses prayed.json holds (the Latinist's input), and
  2. runs checks.py's own code, unchanged, with the Latin restricted to those verses → ps017/checks.md,
     after confirming that the translated ids are an unbroken run of DO's ids from 17:2, in DO's order.
