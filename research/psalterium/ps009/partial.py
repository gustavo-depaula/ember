"""Checks and critic inputs for Ps 9 while it is PARTIAL — copied from ps118/partial.py (left untouched) and adapted.

Run from the repo root, after render.py:  python3.13 research/psalterium/ps009/partial.py
...
This script
  1. trims ps009/latin.json to the verses prayed.json holds (the Latinist's input), and
  2. runs checks.py's own code, unchanged, with the Latin restricted to those verses → ps009/checks.md,
     after confirming that the translated ids are an unbroken run of DO's ids from 9:2, in DO's order.
