"""Critic inputs for a verse range of Ps 9, so that the blind readers read only the new portion.
Copied from ps118/part3.py (left untouched) and adapted: the range is given as DO verse numbers.

Run from the repo root, after render.py and partial.py:
  python3.13 research/psalterium/ps009/part.py 22 39 part2      (9:22–9:39 → ps009/part2/)
...
(the Portuguese alone, for the ambiguity reader). Then, one at a time (k = the version the critics read):
  python3.13 research/psalterium/codex.py research/psalterium/ps009/part2 ../../prompts/latinist.md prayed.vos.json ../critic/v<k>.latinist.part2.json
  python3.13 research/psalterium/codex.py research/psalterium/ps009/part2 ../../prompts/stylist.md prayed.vos.json ../critic/v<k>.stylist.part2.json
  python3.13 research/psalterium/codex.py research/psalterium/ps009/part2/blind ../../../prompts/ambiguity.md prayed.vos.json ../../critic/v<k>.ambiguity.part2.json
...
# the full Latin, not ps009/latin.json (partial.py trims that one to what is translated)
fullLatin = {v['id']: v['text'] for v in latinModule.readVerses(latinModule.doLatin / 'Psalmorum/Psalm9.txt')}
