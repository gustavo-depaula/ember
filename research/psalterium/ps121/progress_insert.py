"""Insert ps121's PROGRESS row in psalm order (after the last row numbered below 121, past the header). Re-reads and writes in one go."""
import json
import re
...
if any(line.startswith('| 121 |') for line in lines):
    print('already there')
    raise SystemExit
at = None
for i, line in enumerate(lines):
    m = re.match(r'\| (\d+) \|', line)
    if m and int(m.group(1)) < 121 and i > 10:
        at = i
