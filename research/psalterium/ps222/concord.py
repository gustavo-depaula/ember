"""Latin regex (accent-folded) -> each finished psalm's verse with its prayed Portuguese (Ps 222 lookups).
Adapted from ps150/concord.py: accent folding, and the folder name printed beside each id.
python3.13 research/psalterium/ps222/concord.py '<regex>' [...]"""
import json
import re
import sys
import unicodedata
...
for folder in sorted(root.glob('ps[0-9][0-9][0-9]')):
    ...
    if not lat.exists() or not vos.exists() or folder.name == 'ps222':
        continue
