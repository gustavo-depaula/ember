"""Latin regex (accent-folded) -> each finished psalm's verse with its prayed Portuguese (Ps 148 lookups).
Adapted from ps131/concord.py, with accent folding.
python3.13 research/psalterium/ps148/concord.py '<regex>' [...]"""
import json
import re
import sys
import unicodedata
...
for folder in sorted(root.glob('ps[0-9][0-9][0-9]')):
    ...
    if not lat.exists() or not vos.exists() or folder.name == 'ps148':
        continue
