"""Latin regex (accent-folded) -> each finished psalm's verse with its prayed Portuguese (Ps 221 lookups).
Adapted from ps150/concord.py: accent folding, and the folder name printed beside each id.
python3.13 research/psalterium/ps221/concord.py '<regex>' [...]"""
import json
import re
import sys
import unicodedata
...
for folder in sorted(root.glob('ps[0-9][0-9][0-9]')):
    ...
    if not lat.exists() or not vos.exists() or folder.name == 'ps221':
        continue
