...
### 1. New collection: `collection/ember-monastic`

Add a single practice `practice/diurnale-monasticum` plus a curated `content/collections/ember-monastic.json` that references it (and any future Nocturnale / Antiphonale). v2 has no library boundary at the source layer — content goes flat by kind, and a collection is just a manifest with `items: [{ ref }]`.

The Diurnale is large (9 data files, seasonal propers), Latin-only, and opt-in. In v2, "opt-in" is the default — users only fetch a practice's blobs when they open it or pin it. No bundling concern.

### 2. Directory structure

```
content/practices/diurnale-monasticum/
  manifest.json
  flow.json
  data/
...
content/collections/ember-monastic.json
  # { "items": [{ "ref": "practice/diurnale-monasticum" }] }
```

Shape mirrors `content/practices/divine-office/data/` exactly. The build pipeline hashes each data file individually so a typo fix in one antiphon set ships ~one blob, not the whole practice.

### 3. Benedictine psalter (the core differentiator)

`psalter-benedictine.json` must encode RB 8–18 — a 1-week cycle keyed by weekday, with Benedictine psalm splits (Ps 9, 17, 67, 77, 103–106, 117, 118 in 22 sections, 143). Use sub-keys like `"17a"`, `"17b"` or `{ref:17, range:"1-13"}`. Hand-coded from the Rule text (mechanical, ~200 entries). Vulgate numbering via `contextKey: "numbering"` — matches existing `psalter-30-day.json` at `content/practices/divine-office/data/psalter-30-day.json`.

### 4. flow.json

Top-level `select on: "hour"` mapping clock ranges to hour branches (pattern from `content/practices/little-office-bvm/flow.json:10-20`):
...
8. Closing: Pater Noster (`{ "ref": "our-father" }` — corpus-wide global resolution, no build-time vendoring), versicles
...
Model on `content/practices/divine-office/manifest.json`:
...
Drop the data files into `content/practices/diurnale-monasticum/data/` and the next `pnpm build:corpus` picks them up — each becomes its own hash-addressed blob referenced from the practice item-manifest.
...
- `content/practices/divine-office/flow.json` — closest structural analogue
- `content/practices/little-office-bvm/flow.json` — hour-by-clock `select` pattern
- `content/practices/divine-office/data/psalter-30-day.json` — data file shape to mirror
- `content/practices/divine-office/manifest.json` — manifest shape to mirror
...
1. `pnpm build:corpus` emits the practice and `collection/ember-monastic` blobs cleanly into `_site/hearth/v2/`; `catalog.json` lists `practice/diurnale-monasticum` and `collection/ember-monastic`.
