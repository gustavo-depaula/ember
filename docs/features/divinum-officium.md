...
- M5: in progress. Porting map (validated against the Perl):
  ...
  - Goldens: Cofficium.pl harness (works since M1) — `command=pray<Hora>`; compare via the same word-stream normalizer as the Mass differential (strip the Ante/Post framing; note /:…:/ inline rubric markers and Comment.txt {suffixes} need normalizing).
- M6–M8: not started.
