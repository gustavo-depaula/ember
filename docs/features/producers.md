...
✅ **Shipped as two producers.** The **EF** Mass is `producer/mass` (`apps/app/src/sources/mass-flow.ts`) — `buildEFFlow()` in `@ember/mass`, slot-centric, with the liturgical *text* as declarative fragments (`mass-fragments.json`) the builder `call`s. The **OF** Mass was rebuilt (2026-06) into its own producer `producer/mass-of` (`apps/app/src/sources/of-mass-flow.ts` + `sources/of/`) that builds **straight to primitives — no engine, no fragments**: it walks an Ordo **spine** (`sources/of/spine.ts`) weaving the Order of Mass with the day's propers, with conditional steps (Gloria, Creed, sprinkling, prayer-over-people) and pick-one **chip selectors** (Penitential Act forms, Greeting, etc.). The Order of Mass is carved from the upstream missal into per-moment `OrderItem`s with a recursive `segments` model. See `docs/mass-rework-journal.md` for the full model. The rest of this section is the original (EF-shaped) spec, kept for context.

> **Cache gotcha (learned the hard way).** A producer's output is cached in SQLite keyed by `producerVersion` + prefs (`runCachedSource.ts`), with **no TTL**. The mass producers read corpus blobs whose hashes change on every build, but the key doesn't include them — so a static `version` renders the *old* corpus forever after a rebuild. Both mass producers therefore set `version` to embed the catalog's `generated` stamp (`get version() { return \`2:${getCatalog().generated}\` }`), so a rebuilt corpus auto-invalidates the cached flow. Any producer that reads the corpus must do the same.
...
| `producer/office-of-readings-reading` | iBreviary (via `producer/breviary-of-the-day`) | ✅ **Shipped** — `apps/app/src/sources/ibreviary/office-of-readings-reading.ts`. Today's **patristic / second reading** from the Office of Readings, lifted out of the full hour: composes `breviary-of-the-day` (`hour: office-of-readings`), then slices the `Primitive[]` from the second-reading rubric (`SECOND READING` / `SEGUNDA LEITURA` / `LECTIO ALTERA`) to the following responsory. Reuses the breviary source's session queue + per-day cache; native-only (web placeholder passes through); `dateScoped`. Surfaces as the `patristic-reading` practice. |
