...
- **EPUBs are embedded inside `.pray` packages, not delivered separately.** Rather than building a separate EPUB delivery system, prose books (spiritual classics) are embedded as EPUB files inside the existing `.pray` zip format. XHTML sources are committed to `content/books/{bookId}/epubs/{epubId}/{lang}/` and `build-books.sh` generates valid EPUB 3 files (one per language) at deploy time. The `.pray` zip includes only `epub.json` (metadata) and the built `.epub` files — XHTML sources are excluded from the archive. This keeps the existing book distribution infrastructure (registry, download, install) unchanged while adding prose book support. The epub.js WebView reader is not yet built — tapping an EPUB shows a placeholder.

- **One EPUB per language, not multi-rendition.** Multi-rendition EPUB (one file, multiple rootfiles in container.xml) was rejected: epub.js support is spotty, it makes EPUBs non-standard for most readers, and it blocks future per-language CDN downloads. Instead, each language gets its own standard EPUB file (e.g., `true-devotion.en-US.epub`, `true-devotion.pt-BR.epub`). The app picks the right file based on the user's content language preference.
