...
- **Content loader stays practice-agnostic by preferring canonical `flow.json` generically (2026-04-14).** Instead of adding practice ID branches in code, filesystem source loading now resolves any practice flow by trying top-level `flow.json` first, then manifest-declared path as fallback. This supports unified-flow migration without hardcoding per-practice rules.

- **Legacy Liguori sidecar flow removed; generic fallback migration kept in loader (2026-04-14).** `practices/meditacoes-ligorio/flows/daily.json` was removed to avoid split truth and stale fixed-slot placeholders (`feastLabel`, `meditation-feast`). Filesystem loading now remains practice-agnostic while supporting legacy manifests/flows through structural detection + migration to canonical `resolve` + `from` semantics.

- **Liguori resolver now includes monthly supplemental meditations on day 25 (2026-04-14).** `resolveLiturgicalMeditation` emits supplemental entries for the 25th-day monthly set, and the content-engine strategy includes those chapters in the `meditations` array. This restores expected multi-meditation days on the 25th while keeping logic in the liturgical kernel instead of app-layer hooks.

- **Day-25 rendering fix required reserve fallback for temporal holes (2026-04-14).** Some 25th dates (e.g., during Pentecost-transition gaps) had no temporal chapter in the map, so only the monthly supplemental meditation appeared. `resolveLiturgicalMeditation` now falls back to `reserves` when temporal lookup fails, preserving the expected “temporal + monthly 25th” pairing.
