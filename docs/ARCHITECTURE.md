...
| `do-data` | `do-data/horas-sancti` | Divinum Officium datasets (EF Mass + Divine Office source files as structured JSON, per-language blobs behind a path index) — see `docs/features/divinum-officium.md` |
...
├── do/                                   # Divinum Officium upstream repo (git submodule, pinned — never edited)
...
| `apps/app/src/content/resolver.ts` | Public surface for the rest of the app. Sync APIs (`resolvePrayer`, `getManifest`) read from the always-resident manifest set. Async APIs (`loadFlow`, `loadChapterContent`, `loadMassProper`) fetch on demand and merge per-language Mass-proper blobs back into the multilingual shape callers expect. `canonicalize(id, hintKind)` is a hard filter when `hintKind` is set — no fallthrough across kinds. |
| `apps/app/src/content/books.ts` | The one book module both the practice engine and the reader resolve through: book id → manifest (`loadBook`, fetched on demand; `getResidentBook` once loaded), chapter → authored body (`loadChapterSource`), and manifest images (`findBookImage`). External books (CCC, Escrivá) plug in as chapter producers via `registerChapterProducer`. The engine's sync book hooks stay correct because `resolveFlowAsync` calls `prepareBooks` (→ `loadBooks`) first. |
...
- **`@ember/divinum-officium`** — Pure TS port of the Divinum Officium engine (parser, condition evaluator, reference resolver, kalendar, hour/Mass assembly). Accepts data via a `DoLoader` interface. See `docs/features/divinum-officium.md`.
